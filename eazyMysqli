...
class DB {
    ...
    public function __construct($a = null) {
        if (is_array($a)) {
            ...
            $this->db = (isset($a['database'])) ? $a['database'] : $this->database;
            $this->table = (isset($a['table'])) ? $a['table'] : $this->table;

        } elseif (is_string($a)) {
            if (preg_match("/^(mysqli):\/\/([^:]+):(.*)@([a-z0-9\-.]+):?(\d+)?\/([a-z0-9_\-]+)(\/([a-z0-9_\-]+))?\/?$/i", $a, $m)) {
                $this->engine = $m[1];
                $this->host   = $m[4];
                $this->user   = $m[2];
                $this->pass   = $m[3];
    ...
    private function _select_db() {
        if ($this->engine == 'mysqli') {
            ...
            return mysqli_select_db($this->c , $this->db );
        }
    }
            ...
            if ($r === 1 && !$options['array']) {
                ...
                if ($this->engine == 'mysqli') {
                    ...
                    if ($options['raw']) {
                        ...
                        return new DB_record($this, mysqli_fetch_assoc($q));
                    }
                }
            } else {
                ...
                if ($this->engine == 'mysqli') {
                    // if we're using mysql
                    while (($row = mysqli_fetch_assoc($q)) != false) {
